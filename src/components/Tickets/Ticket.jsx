...
const Ticket = ({ticket, task, setTask}) => {
    ...
    return (
        <div
        onClick={()=> setTask([...task, task.includes(issue_title) ? '' : issue_title])}
         className='p-3 bg-white rounded space-y-3 cursor-alias'>
            <div className='flex justify-between items-center'>
                <h2 className='font-bold text-xl'>{issue_title}</h2>
                <span className='text-[#02A53B] font-medium text-sm bg-[#B9F8CF] p-2 rounded-3xl flex items-center gap-2'><div className='w-4 h-4 bg-[#02A53B] rounded-full'></div>Open</span>
            </div>
